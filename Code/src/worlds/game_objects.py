...
class Client(Entity):
    ...
    def interact(self,agent,action):
        #waiter gives order and client goes away
        if self.in_map and agent.has_order and self.has_ordered:
            agent.has_order=False
            self.in_map=False
            self.has_ordered=False
        #client decide what to eat
        if self.in_map and not agent.has_order and not self.has_ordered:
            o = random.choice(self.orders)

            while o.in_map==True:
                o=random.choice(self.orders)#cannot be the same order object of another client
            o.in_map=True
...
class Order(Entity):
    ...
    def interact(self,agent,action):
        if self.in_map and not agent.has_order:
            agent.has_order=True
            self.in_map=False
        return not self.in_map
